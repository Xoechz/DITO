...
const ENTITY_KEY_VALUE = "swie_mon.key"
const JOB_KEY_VALUE = "swie_mon.job_id"
...
func TestTracesConnector(t *testing.T) {
	...
	t.Run("no entity spans", func(t *testing.T) {
		...
		connector.cache = make(map[string]*swie_monCacheEntry)
...
func TestMetricsConnector(t *testing.T) {
	...
	t.Run("grouped metrics", func(t *testing.T) {
		...
		assert.Equal(t, "swie_mon.entity.count", outputMetrics.At(0).Name())
		...
		for _, dp := range dataPoints.All() {
			...
			status, statusExists := dp.Attributes().Get("swie_mon.entity.status_code")
...
func TestConnectorCapabilities(t *testing.T) {
	t.Run("swie_mon traces connector capabilities", func(t *testing.T) {
		connector := &swie_monTracesConnector{}
		capabilities := connector.Capabilities()
		assert.False(t, capabilities.MutatesData)
	})

	t.Run("swie_mon metrics connector capabilities", func(t *testing.T) {
		connector := &swie_monMetricsConnector{}
		capabilities := connector.Capabilities()
		assert.False(t, capabilities.MutatesData)
	})
}
