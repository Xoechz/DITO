...
const SWIE_MON_SERVICE_NAME = "swie_mon"

type swie_monEntitySpan struct {
	entityKey  string
	span       ptrace.Span
	receivedAt time.Time
}

type swie_monJobSpan struct {
	jobKey string
	span   ptrace.Span
}

type swie_monCacheEntry struct {
...
type swie_monTracesConnector struct {
	logger        *zap.Logger
	config        Config
	...
	cache       map[string]*swie_monCacheEntry
	entitySpans map[pcommon.SpanID]*swie_monEntitySpan
	mutex       sync.RWMutex
}

type swie_monMetricsConnector struct {
	logger          *zap.Logger
	config          Config
...
func newTracesConnector(logger *zap.Logger, config component.Config, nextConsumer consumer.Traces) (*swie_monTracesConnector, error) {
	logger.Info("Building swie_mon traces connector")
	cfg := config.(*Config)

	return &swie_monTracesConnector{
		config:        *cfg,
		logger:        logger,
		traceConsumer: nextConsumer,
		cache:         make(map[string]*swie_monCacheEntry),
		entitySpans:   make(map[pcommon.SpanID]*swie_monEntitySpan),
	}, nil
}

func newMetricsConnector(logger *zap.Logger, config component.Config, nextConsumer consumer.Metrics) (*swie_monMetricsConnector, error) {
	logger.Info("Building swie_mon metrics connector")
	cfg := config.(*Config)

	return &swie_monMetricsConnector{
		config:          *cfg,
		logger:          logger,
...
func (s *swie_monTracesConnector) Capabilities() consumer.Capabilities {
	return consumer.Capabilities{MutatesData: false}
}

func (s *swie_monMetricsConnector) Capabilities() consumer.Capabilities {
	return consumer.Capabilities{MutatesData: false}
}

func (s *swie_monTracesConnector) ConsumeTraces(ctx context.Context, td ptrace.Traces) error {
	...
	for _, span := range newEntitySpans {
		s.entitySpans[span.span.SpanID()] = &swie_monEntitySpan{
	...
	rs.Resource().Attributes().PutStr("service.name", SWIE_MON_SERVICE_NAME)
	rs.Resource().Attributes().PutInt("swie_mon.lock_wait_duration", lockWaitTime.Milliseconds())
	rs.Resource().Attributes().PutInt("swie_mon.entity.pending", int64(entitySpanCount))
	scopeSpan := rs.ScopeSpans().AppendEmpty()
	scopeSpan.Scope().SetName(SWIE_MON_SERVICE_NAME)
	...
	rs.Resource().Attributes().PutInt("swie_mon.entity.consumed", int64(len(consumedSpanIds)))
...
func (s *swie_monMetricsConnector) ConsumeTraces(ctx context.Context, td ptrace.Traces) error {
	...
	rm.Resource().Attributes().PutStr("service.name", SWIE_MON_SERVICE_NAME)

	sm := rm.ScopeMetrics().AppendEmpty()
	sm.Scope().SetName(SWIE_MON_SERVICE_NAME)

	countMetric := sm.Metrics().AppendEmpty()
	countMetric.SetName("swie_mon.entity.count")
	...
	for group, count := range metricGroups {
		...
		dp.Attributes().PutStr("swie_mon.entity.status_code", group.statusCode.String())
...
func (s *swie_monTracesConnector) ConsumeSpan(jobSpans map[pcommon.SpanID]swie_monJobSpan, scopeSpan ptrace.ScopeSpans, entitySpan *swie_monEntitySpan) bool {
...
func (s *swie_monTracesConnector) sweepCache() {
...
func gatherSpans(rss ptrace.ResourceSpansSlice, config *Config) (entitySpans []swie_monEntitySpan, jobSpans map[pcommon.SpanID]swie_monJobSpan) {
	entitySpans = make([]swie_monEntitySpan, 0)
	jobSpans = make(map[pcommon.SpanID]swie_monJobSpan)
...
func checkEntitySpan(span ptrace.Span, config *Config) (bool, *swie_monEntitySpan) {
	...
	return true, &swie_monEntitySpan{
		entityKey: entityKey.AsString(),
		span:      span,
	}
}

func checkJobSpan(span ptrace.Span, config *Config) (bool, *swie_monJobSpan) {
